...
public class StatusController : ControllerBase
{
    ...
    public async Task<IActionResult> Messages(string systemName, int order = 0, int count = 10, int offset = 0)
    {
        try
        {
            ...
            if (order == 1) messagesForSystem = messagesForSystem.OrderByDescending(m => m.Id).ToList();
    ...
    public async Task<IActionResult> MessagesByDateCreated(string systemName, DateTime from, DateTime to, int order = 0,
        int count = 1)
    {
        ...
        try
        {
            ...
            var messagesForSystem = messages.ToList();
            if (order == 1) messagesForSystem = messagesForSystem.OrderByDescending(m => m.UpdatedAt).ToList();
